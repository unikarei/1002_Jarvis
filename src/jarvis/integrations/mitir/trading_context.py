"""JARVIS-owned bounded, read-only Trading Context use case."""
...
class TradingContextService:
    ...
    def get_context(self, limit: int = DEFAULT_TRADING_LIMIT) -> TradingContextResult:
        if not 1 <= limit <= 100:
            raise ValueError("limit must be between 1 and 100")
        task, correlation_id = self._runner.run("trading.context", {"limit": limit}, requester="jarvis")
        return map_trading_context_result(task, correlation_id)


def trading_context_service_from_environment(environment: dict[str, str] | None = None) -> TradingContextService:
    ...
    try:
        return TradingContextService(ReadOnlySpecialistRunner(MiTiRClient(base_url, token)))
    except ValueError as exc:
        raise DailySummaryError(DailySummaryFailureCategory.CONFIGURATION, "MiTiR runtime configuration is invalid") from exc


def map_trading_context_result(task: TaskRecord, correlation_id: str) -> TradingContextResult:
    if not isinstance(task.result, dict):
        raise DailySummaryError(DailySummaryFailureCategory.CONTRACT, "MiTiR Trading context result was invalid")
    result = task.result
    return TradingContextResult(
        ...
        mode=_text(result, "mode"), activity=_items(result, "important_items"), alerts=_items(result, "alerts"),
        source_references=_items(result, "source_artifacts"), task_id=str(task.id), correlation_id=correlation_id,
